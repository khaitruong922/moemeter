...
export const BookCard: React.FC<Props> = ({
	...
	const bookUsers = hideBookUsers ? [] : book.user_ids.map((id) => users[id]).filter(Boolean);
	...
	const userCountLabel = isLonely ? 'ひとり' : `${book.user_ids.length}人`;
		...
		<div className="bg-white overflow-hidden">
			<div className="flex items-start gap-2 sm:gap-4 p-2 sm:p-4">
				<div className="flex-none w-12 sm:w-16 text-center flex flex-col justify-between">
					<div>
						...
						<div className="text-xs sm:text-sm text-gray-500">{userCountLabel}</div>
						{book.date && (
			...
			{hasReviews && showReviews && (
				<div className="w-full px-2 sm:px-4 pb-2 sm:pb-4">
					<ReviewList reviews={book.reviews} isRead={isRead} />
				</div>
			)}
		</div>
	);
